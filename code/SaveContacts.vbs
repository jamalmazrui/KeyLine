Option Explicit
WScript.Echo "Starting SaveContacts"

Dim dExtensions, dFolders
Dim iContactMessage, iMailBox, iAttachmentTotal, iMessage, iMessageCount, iAttachment, iAttachmentCount
Dim oApp, oExplorer, oMessages, oMessage, oAttachments, oAttachment, oNameSpace, oFolder
Dim sExtensions, sFilter, sVcfFile, sMessage, sRoot, sExt, sAction, sFolder, sBaseName, sFileName, sDir
...
sTargetLog = PathCombine(PathGetCurrentDirectory(), "Contacts.log")
...
printBlank
print "Saving " & StringPlural("contact", iMessageCount)

For iMessage = 1 to iMessageCount
Set oMessage = oMessages(iMessage)
sMessage = ""
On Error Resume Next
sMessage = Trim(oMessage.FirstName & " " & oMessage.LastName & ", " & oMessage.CompanyName)
On Error GoTo 0
If Len(sMessage) > 0 Then
If Right(sMessage, 1) = "," Then sMessage = Trim(StringChopRight(sMessage, 1))
print iMessage & ". " & sMessage

sRoot = sMessage
sExt = "vcf"
sVcfFile = PathGetUnique(sDir, sRoot, sExt)
oMessage.SaveAs sVcfFile, olVCard  
sVcfFile = PathChangeExtension(sVcfFile, "txt")
oMessage.SaveAs sVcfFile, olTXT    
End If ' sMessage not blank
Next
printBlank
