...
Dim iRow, iRowCount, iCol, iColCount, i, iLevel, iReplaceCount, iTableId, iReplace, iWrap, iForward, iArgCount, iCount, iLowerHeadingLevel, iUpperHeadingLevel
Dim oRow, oCell, oSystem, oFile, oParagraph, oField, oAddedStyles, oApp, oData, oDoc, oDocs, oFind, oFont, oFormat, oProperty, oRange, oReplace, oStyle, oStyles, oToc, oTocs
Dim oTag, oTags
Dim sCell, sSourceName, sTargetRoot, sTargetName, sTargetExtension, sBackupDocx, sTargetLog, sScriptVbs, sHomerLibVbs, sDir, sCode, sFindStyle, sReplaceStyle, sKey, sFind, sFindText, sReplaceText, sValue, sAttrib, s, sStyle, sName, sSourceDocx, sMatch, sTargetIni, sText, sConfigFile, sSourceIni, sSection
...
' if oShape.Decorative then sReturn = sReturn & "Decorative: True" & " \" & vbCrLf
...
sReturn = AddField(sReturn, "Size", iFontSize)
...
sReturn = addField(sReturn, "Columns", iColCount)
sReturn = addField(sReturn, "Rows", iRowCount)
...
sTargetRoot = "meta-" & PathGetRoot(sSourceName)
...
sText = sText & vbcrlf
sText = sText & "## Slide " & oSlide.SlideIndex & ": " & dLayoutType(oSlide.Layout) & vbCrLf & vbCrLf
sText = AddField(sText, "Number", oSlide.SlideNumber)
...
sText = sText & vbCrLf
s = StringPlural("Shape", iShapeCount)
